...
type SessionConfig struct {
	...
	// VerifyTopUpTx accepts both the state-delta verifier used by current Go
	// integrations and the legacy payload-only SessionTxVerifier callback.
	// New integrations should also install VerifyTopUpStateTx so the resulting
	// account is bound to the persisted channel identity.
	VerifyTopUpTx any
...
func (s *SessionServer) ProcessTopUp(ctx context.Context, payload *intents.TopUpPayload) (ChannelState, error) {
	...
	// Bind the resulting on-chain state to the identity already persisted for
	// this channel before raising its local deposit.
	if s.config.VerifyTopUpTx != nil {
		if err := invokeTopUpVerifier(ctx, s.config.VerifyTopUpTx, payload, verifiedDeposit); err != nil {
...
func invokeTopUpVerifier(ctx context.Context, verifier any, payload *intents.TopUpPayload, currentDeposit uint64) error {
	switch typed := verifier.(type) {
	case TopUpTxVerifier:
		if typed == nil {
			return nil
		}
		return typed(ctx, payload, currentDeposit)
	case func(context.Context, *intents.TopUpPayload, uint64) error:
		return typed(ctx, payload, currentDeposit)
	case SessionTxVerifier[intents.TopUpPayload]:
		if typed == nil {
			return nil
		}
		_, err := typed(ctx, payload)
		return err
	case func(context.Context, *intents.TopUpPayload) (string, error):
		_, err := typed(ctx, payload)
		return err
	default:
		return fmt.Errorf("unsupported top-up verifier type %T", verifier)
	}
}
