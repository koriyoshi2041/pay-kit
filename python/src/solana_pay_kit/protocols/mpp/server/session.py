...
class SessionConfig:
    ...
    # Legacy hook supports both the original payload-only callback and the
    # state-aware callback used by transaction-delta verifiers.
    verify_top_up_tx: SessionTxVerifier[TopUpPayload] | SessionTopUpTxVerifier | None = None
...
class SessionServer:
    ...
    async def process_top_up(self, payload: TopUpPayload) -> ChannelState:
        ...
        verified_deposit = current.deposit

        if self._config.verify_top_up_tx is not None:
            try:
                verifier = self._config.verify_top_up_tx
                if len(inspect.signature(verifier).parameters) >= 2:
                    await verifier(payload, current)  # type: ignore[call-arg]
                else:
                    await verifier(payload)  # type: ignore[call-arg]
            except PaymentError:
                raise
            except Exception as exc:
        ...
        def mutator(current: ChannelState | None) -> ChannelState:
            ...
            if (
                (self._config.verify_top_up_tx is not None or self._config.verify_top_up_state_tx is not None)
                and current.deposit != verified_deposit
            ):
