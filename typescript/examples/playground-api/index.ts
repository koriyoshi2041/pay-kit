...
const pay = await createPayKit({
    ...
    pricing: {
        ...(PLAN
            ? {
                  feed: subscription(usd('0.10'), {
                      ...
                      periodCount: 1,
                      periodUnit: 'day',
                      planBump: PLAN.planBump,
                      planCreatedAt: PLAN.planCreatedAt,
                      planId: PLAN.planId,
                      planIdNumeric: PLAN.planIdNumeric,
        ...
        stream: session(usd('1.00'), {
            closeDelayMs: 2000,
            description: 'Metered token stream',
            unitPrice: usd('0.0001'),
        }),
...
app.post(
    ...
    (req: Request, res: Response) => {
        ...
        res.json({
            billedBaseUnits: (tokens * PRICE_PER_TOKEN).toString(),
            summarizedBytes: body.length,
